...
trait VerilogDebugBundle extends Bundle {
...
class core_cpu extends BlackBox {
  val io = IO(new Bundle with VerilogDebugBundle{
...
class CPU(idBits: Int)(implicit p: Parameters) extends LazyModule {
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    val cpu = Module(new core_cpu)
    cpu.io.aclk := clock
    cpu.io.aresetn := reset
